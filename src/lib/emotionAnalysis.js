...
Format your response as valid JSON only, no additional text.`;
    ...
    try {
      ...
      emotionData = JSON.parse(jsonMatch ? jsonMatch[1] || jsonMatch[0] : content);
    } catch (parseError) {
...
function adjustColorIntensity(hexColor, factor) {
  ...
  // Adjust brightness
  const adjust = (value) => {
    const adjusted = Math.round(value * (0.5 + factor * 0.5));
...
export function detectEmotionPatterns(messages) {
  ...
  const patterns = [];
  const recentEmotions = messages.slice(-5).map(m => m.tone_analysis?.primaryEmotion || 'neutral');
...
export function generateEmotionInsights(emotionStats) {
  ...
  if (emotionStats.positive_ratio > 0.7) {
    insights.push({
      type: 'positive',
      message: 'Your communication is predominantly positive! Keep it up.',
      icon: '😊'
    });
  } else if (emotionStats.positive_ratio < 0.3) {
    insights.push({
      type: 'warning',
      message: 'Consider focusing on more positive expressions to improve communication.',
      icon: '⚠️'
    });
  }
  
  if (emotionStats.emotional_volatility > 3) {
    insights.push({
      type: 'info',
      message: 'Your emotions are varying significantly. Try to maintain consistency.',
      icon: 'ℹ️'
    });
  }
  
  if (emotionStats.average_intensity > 7) {
    insights.push({
      type: 'info',
      message: 'Your emotional intensity is high. Consider taking calming breaths.',
      icon: '🧘'
    });
  }
